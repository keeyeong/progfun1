...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def descByRetweetIter(tweets: TweetSet, prev: TweetList): TweetList = {
      if (tweets.isEmpty) prev
      else {
        val t = tweets.mostRetweeted
        new Cons(t, descByRetweetIter(tweets.remove(t), prev))
      }
    }

    descByRetweetIter(this, Nil)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted = {
    ...
    if ((left.isEmpty && right.isEmpty) || (left.isEmpty && elem.retweets > rightMost.retweets) || (right.isEmpty && elem.retweets > leftMost.retweets)) elem
    else if (left.isEmpty) rightMost
    else leftMost
  }
